...
from seq_queries.data import load_mooc_data, process_app_mooc_data
...
def train(args,train_dataloader, valid_dataloader):
    ...
    while epoch < args.train_epochs:
        results["train"].append(train_epoch(args, model, optimizer, lr_scheduler, train_dataloader, epoch+1,disable_tqdm=args.disable_tqdm))
...
def main():
    ...
    args = get_args(manual_config="/home/showalte/research/prob_seq_queries/archive/experiments/train/moocs.yaml")
    ROOT = os.path.normpath(os.path.join(__file__,"../../../"))
    DEVICE = args.device
    DISABLE_TQDM = args.disable_tqdm
    # text_dict= load_text(args.data_path)
    text_dict= load_mooc_data(args.data_path,args)
    args.text_dict = text_dict
    print(text_dict['char_to_id'])
    print("====="*10, flush=True)
    # train_dl, val_dl, test_dl = process_text_data(text_dict, args)
    train_dl, val_dl, test_dl = process_app_mooc_data(text_dict, args)
    ...
    save_path =os.path.join(ROOT,"models/moocs/")
    os.makedirs(save_path,exist_ok = True)
    torch.save(model.state_dict(), f"{save_path}/mooc_model.pt")
